    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.projects.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('paas_code') ? 'is-invalid' : '' }}" type="text" name="paas_code" id="paas_code" value="{{ old('paas_code', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('approval_status') ? 'is-invalid' : '' }}" name="approval_status" id="approval_status" required>
                    ...
                    @foreach(App\Models\Project::APPROVAL_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('approval_status', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('fund') ? 'is-invalid' : '' }}" name="fund_id" id="fund_id" required>
                    @foreach($funds as $id => $entry)
                        <option value="{{ $id }}" {{ old('fund_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('fund'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('pag_value') ? 'is-invalid' : '' }}" type="number" name="pag_value" id="pag_value" value="{{ old('pag_value', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lead_organisation_unit') ? 'is-invalid' : '' }}" name="lead_organisation_unit_id" id="lead_organisation_unit_id" required>
                    @foreach($lead_organisation_units as $id => $entry)
                        <option value="{{ $id }}" {{ old('lead_organisation_unit_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('donors') ? 'is-invalid' : '' }}" name="donors[]" id="donors" multiple required>
                    @foreach($donors as $id => $donor)
                        <option value="{{ $id }}" {{ in_array($id, old('donors', [])) ? 'selected' : '' }}>{{ $donor }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_expenditure') ? 'is-invalid' : '' }}" type="number" name="total_expenditure" id="total_expenditure" value="{{ old('total_expenditure', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_contribution') ? 'is-invalid' : '' }}" type="number" name="total_contribution" id="total_contribution" value="{{ old('total_contribution', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_psc') ? 'is-invalid' : '' }}" type="number" name="total_psc" id="total_psc" value="{{ old('total_psc', '') }}" step="0.01" required>
