    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.themes.update", [$theme->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.theme.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $theme->name) }}" required>
                @if($errors->has('name'))
